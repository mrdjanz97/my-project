...
var CommandType = /* @__PURE__ */ ((CommandType2) => {
  CommandType2[CommandType2["signIn"] = 0] = "signIn";
  CommandType2[CommandType2["signOut"] = 1] = "signOut";
  CommandType2[CommandType2["getSession"] = 2] = "getSession";
  CommandType2[CommandType2["signUp"] = 3] = "signUp";
  CommandType2[CommandType2["fetch"] = 4] = "fetch";
  return CommandType2;
})(CommandType || {});
const resolveFetch$3 = (customFetch) => {
  ...
  if (customFetch) {
    ...
    _fetch = (...args) => __vitePreload(() => Promise.resolve().then(() => browser$2), true ? void 0 : void 0).then(({ default: fetch2 }) => fetch2(...args));
  } else {
    _fetch = fetch;
  }
...
const convertChangeData = (columns, record, options2 = {}) => {
  var _a;
...
const resolveResponse = () => __awaiter$5(void 0, void 0, void 0, function* () {
  if (typeof Response === "undefined") {
    return (yield __vitePreload(() => Promise.resolve().then(() => browser$2), true ? void 0 : void 0)).Response;
  }
  return Response;
});
...
const _getRequestParams$1 = (method, options2, parameters, body) => {
...
function _handleRequest$1(fetcher, method, url, options2, parameters, body) {
  return __awaiter$4(this, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      fetcher(url, _getRequestParams$1(method, options2, parameters, body)).then((result) => {
        if (!result.ok)
          throw result;
...
function get(fetcher, url, options2, parameters) {
  return __awaiter$4(this, void 0, void 0, function* () {
    return _handleRequest$1(fetcher, "GET", url, options2, parameters);
  });
}
function post(fetcher, url, body, options2, parameters) {
  return __awaiter$4(this, void 0, void 0, function* () {
    return _handleRequest$1(fetcher, "POST", url, options2, parameters, body);
  });
}
function put(fetcher, url, body, options2, parameters) {
  return __awaiter$4(this, void 0, void 0, function* () {
    return _handleRequest$1(fetcher, "PUT", url, options2, parameters, body);
  });
}
function remove(fetcher, url, body, options2, parameters) {
  return __awaiter$4(this, void 0, void 0, function* () {
    return _handleRequest$1(fetcher, "DELETE", url, options2, parameters, body);
  });
}
...
const _getRequestParams = (method, options2, parameters, body) => {
  const params = { method, headers: (options2 === null || options2 === void 0 ? void 0 : options2.headers) || {} };
  if (method === "GET") {
    return params;
  }
...
async function _request(fetcher, method, url, options2) {
  ...
  const data = await _handleRequest(fetcher, method, url + queryString, { headers, noResolveJson: options2 === null || options2 === void 0 ? void 0 : options2.noResolveJson }, {}, options2 === null || options2 === void 0 ? void 0 : options2.body);
...
async function _handleRequest(fetcher, method, url, options2, parameters, body) {
  const requestParams = _getRequestParams(method, options2, parameters, body);
  let result;
  try {
...
const createClient = (supabaseUrl2, supabaseKey2, options2) => {
  return new SupabaseClient(supabaseUrl2, supabaseKey2, options2);
};
const getLocalStorage = async (key) => (await chrome.storage.local.get(key))[key];
const removeLocalStorage = async (key) => await chrome.storage.local.remove(key);
const setLocalStorage = async (dataObject) => await chrome.storage.local.set(dataObject);
const storageAdapter = {
  getItem: async (name2) => {
    return await getLocalStorage(name2);
  },
  setItem: async (name2, value) => {
    return await setLocalStorage({ [name2]: value });
  },
  removeItem: async (name2) => {
    return await removeLocalStorage(name2);
  }
};
const options = {
  auth: {
...
const supabaseUrl = "https://wuhzqfhprtcqnemkrqqj.supabase.co";
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind1aHpxZmhwcnRjcW5lbWtycXFqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjE4MTU5OTQsImV4cCI6MjAzNzM5MTk5NH0.D5MqdY_XwYwof4fl6O16j-orRHtLQwGkvx5BwP_7lig";
const supabase = createClient(supabaseUrl, supabaseKey, options);
const googleUserInfoUrl = "https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=";
const accessTokenParam = "#access_token";
const idTokenParam = "id_token";
const google = "google";
const PROFILES_TABLE = "profiles";
const FEEDBACKS_TABLE = "feedbacks";
const FEEDBACK_REACTIONS_TABLE = "feedback_reactions";
const GET_FEEDBACKS_WITH_REACTIONS = "get_feedbacks_with_reactions";
const COMPANY_RESOURCES = "company_resources";
const NOTIFICATIONS_TABLE = "notifications";
const FEEDBACK_COMMENTS_TABLE = "feedback_comments";
const NEW_NOTIFICATIONS_COUNT_RPC = "get_new_notifications_count";
const DEV_BUCKET = "dev";
const RESOURCES_BUCKET_FOLDER = "resources";
const AVATARS_BUCKET_FOLDER = "avatars";
async function getUserSession() {
  var _a;
  const session = await supabase.auth.getSession();
  return { data: (_a = session == null ? void 0 : session.data) == null ? void 0 : _a.session, error: null };
}
async function signIn(values) {
  const { data, error } = await supabase.auth.signInWithPassword(values);
  return { data: data == null ? void 0 : data.session, error: error == null ? void 0 : error.message };
}
async function signOut() {
  const { error } = await supabase.auth.signOut();
  return error == null ? void 0 : error.message;
}
async function updateUserRepository({ avatar, userId }) {
  const { data, error } = await supabase.from(PROFILES_TABLE).update({
    avatar
  }).eq("id", userId).select("*");
  return { data, error };
}
const uploadAvatarRepository = async (file) => {
  const timestamp = Date.now();
  const uniqueFileName = `${timestamp}-${file.name}`;
  const { data, error } = await supabase.storage.from(DEV_BUCKET).upload(`${AVATARS_BUCKET_FOLDER}/${uniqueFileName}`, file);
  if (error) {
...
const deleteItemFromBucketRepository = async (path) => {
...
class AuthService {
  ...
  async googleSignInUser(redirectedTo) {
    const url = new URL(redirectedTo);
    const params = new URLSearchParams(url.hash);
    const auth = await supabase.auth.signInWithIdToken({
      provider: google,
    ...
    const data = await this.googleFetchUserInfo(params.get(accessTokenParam));
  ...
  async googleFetchUserInfo(accessToken) {
...
const deleteItemFromBucket = async (file) => {
  try {
...
export {
  ...
  CommandType as C,
  DEV_BUCKET as D,
  FEEDBACK_REACTIONS_TABLE as F,
  GET_FEEDBACKS_WITH_REACTIONS as G,
  NOTIFICATIONS_TABLE as N,
  PROFILES_TABLE as P,
  RESOURCES_BUCKET_FOLDER as R,
  updateUser as a,
  NEW_NOTIFICATIONS_COUNT_RPC as b,
  FEEDBACKS_TABLE as c,
  deleteItemFromBucket as d,
  FEEDBACK_COMMENTS_TABLE as e,
  COMPANY_RESOURCES as f,
  supabase as s,
  uploadAvatar as u
};
